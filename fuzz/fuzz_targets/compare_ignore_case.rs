//! Fuzz target for `fast_natord::compare_ignore_case` (case-insensitive).
//!
//! Splits the AFL-supplied byte slice at the first null byte (or midpoint
//! if absent) and compares the two halves as UTF-8 strings with case folding.
